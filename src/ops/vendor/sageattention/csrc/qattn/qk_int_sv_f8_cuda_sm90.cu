...
// ===================== Packed INT4 + TMA Bandwidth Optimization =====================
// Two int4 values packed per byte in global memory, TMA loads packed data with no swizzle
// to a staging area, then all threads cooperate to unpack to int8 with 128B swizzle for WGMMA.

template<uint32_t ROWS, uint32_t COLS, uint32_t NUM_THREADS>
__device__ __forceinline__ void unpack_int4_to_int8_swizzled(
    const int8_t* __restrict__ packed,
    int8_t* __restrict__ unpacked,
    uint32_t tid)
{
    static_assert(COLS == 64 || COLS == 128);
    constexpr uint32_t packed_cols = COLS / 2;
    static_assert(packed_cols % 4 == 0);
    constexpr uint32_t u32_per_row = packed_cols / 4;
    constexpr uint32_t total_u32 = ROWS * u32_per_row;
    constexpr uint32_t num_iters = total_u32 / NUM_THREADS;

    const uint32_t smem_base = static_cast<uint32_t>(__cvta_generic_to_shared(unpacked));
    const uint32_t base_row = (smem_base / 128) & 7;
    const uint32_t* __restrict__ packed_u32 = reinterpret_cast<const uint32_t*>(packed);

    // Precompute: each thread's column position is CONSTANT across all iterations
    // because row stride = NUM_THREADS / u32_per_row = 8, and swizzle depends on (row & 7)
    // which cycles with period 8 → same swizzle pattern every iteration.
    const uint32_t col_idx = tid % u32_per_row;
    const uint32_t col = col_idx * 8;  // unpacked column = packed_col_idx * 4 * 2
    const uint32_t first_row = tid / u32_per_row;
    const uint32_t swizzled_col = col ^ (((base_row + first_row) & 7) << 4);
    uint32_t dst = first_row * COLS + swizzled_col;

    #pragma unroll
    for (uint32_t i = 0; i < num_iters; i++) {
        uint32_t p4 = packed_u32[tid + i * NUM_THREADS];

        // Extract low and high nibbles (4 bytes at once)
        uint32_t lo = p4 & 0x0F0F0F0Fu;
        uint32_t hi = (p4 >> 4) & 0x0F0F0F0Fu;

        // Sign extend 4-bit to 8-bit using PRMT sign-replicate mode:
        // PTX prmt.b32 with selector nibble bit[3]=1 replicates MSB (bit 7) of the
        // selected source byte to all 8 bits (0xFF if negative, 0x00 if positive).
        // Note: __byte_perm() strips bit[3], so we must use inline PTX.
        // For lo: shift left 4 to move int4 sign (bit 3) to byte MSB (bit 7).
        //   Cross-byte shift is safe because all nibble values are 0-15 (high nibble=0).
        // For hi: p4's bit 7 of each byte IS the int4 sign bit of the high nibble.
        uint32_t lo_shifted = lo << 4;
        uint32_t lo_sign, hi_sign;
        uint32_t sel = 0xBA98u;
        asm("prmt.b32 %0, %1, %2, %3;" : "=r"(lo_sign) : "r"(lo_shifted), "r"(0u), "r"(sel));
        asm("prmt.b32 %0, %1, %2, %3;" : "=r"(hi_sign) : "r"(p4), "r"(0u), "r"(sel));
        lo |= (lo_sign & 0xF0F0F0F0u);
        hi |= (hi_sign & 0xF0F0F0F0u);

        // Interleave: [lo0,hi0,lo1,hi1] and [lo2,hi2,lo3,hi3] using PRMT
        uint32_t out0 = __byte_perm(lo, hi, 0x5140u);
        uint32_t out1 = __byte_perm(lo, hi, 0x7362u);

        *reinterpret_cast<uint32_t*>(&unpacked[dst]) = out0;
        *reinterpret_cast<uint32_t*>(&unpacked[dst + 4]) = out1;
        dst += NUM_THREADS / u32_per_row * COLS;  // advance by row_stride * COLS
    }
}

template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t NUM_THREADS, uint32_t head_dim, QuantGranularity Q_GRAN, QuantGranularity K_GRAN, typename DTypeOut, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_scale=false>
__global__ void qk_int4_packed_sv_f8_attn_kernel(const __grid_constant__ CUtensorMap tensorMapQ_packed,
                                        const __grid_constant__ CUtensorMap tensorMapK_packed,
                                        const __grid_constant__ CUtensorMap tensorMapV,
                                        float *__restrict__ Q_scale, float *__restrict__ K_scale, float *__restrict__ V_scale,
                                        DTypeOut* O, float *__restrict__ Lse, uint32_t stride_bz_o, uint32_t stride_h_o, uint32_t stride_seq_o,
                                        const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
                                        float sm_scale)
{
  ...
  // SMEM layout: sQ | sK[0] | sK[1] | sV | sStaging (double-buffered sK, 64KB total)
  int8_t *sQ = (int8_t*)smem_;
  int8_t *sK_read  = (int8_t*)(smem_ + CTA_Q * head_dim);
  int8_t *sK_write = (int8_t*)(smem_ + CTA_Q * head_dim + CTA_K * head_dim);
  int8_t *sV = (int8_t*)(smem_ + CTA_Q * head_dim + 2 * CTA_K * head_dim);
  int8_t *sStaging = (int8_t*)(smem_ + CTA_Q * head_dim + 2 * CTA_K * head_dim + CTA_K * head_dim);
...
#pragma unroll
  ...
  // Load packed Q to staging area and V to sV
  if (threadIdx.x == 0)
  {
    expect_bytes<(CTA_Q * head_dim / 2) * sizeof(int8_t)>(&barrier_Q);
    expect_bytes<(CTA_K * head_dim) * sizeof(int8_t)>(&barrier_V);
    load_async_4D(sStaging, &tensorMapQ_packed, &barrier_Q, 0, bx * CTA_Q, head_id, batch_id);
    load_async_4D(sV, &tensorMapV, &barrier_V, 0, 0, kv_head_id, batch_id);
  }
  ...
  // Wait for packed Q, unpack to sQ with 128B swizzle
  wait(&barrier_Q, 0);
  unpack_int4_to_int8_swizzled<CTA_Q, head_dim, NUM_THREADS>(sStaging, sQ, threadIdx.x);
  __syncthreads();  // needed: TMA K_0 writes to sStaging next
  ...
  // Load packed K_0 to staging, wait, unpack to sK_read (buffer 0)
  if (threadIdx.x == 0)
  {
    expect_bytes<(CTA_K * head_dim / 2) * sizeof(int8_t)>(&barrier_K);
    load_async_4D(sStaging, &tensorMapK_packed, &barrier_K, 0, 0, kv_head_id, batch_id);
  }
  int k_phase = 0;
  int v_phase = 0;
  wait(&barrier_K, k_phase);
  k_phase ^= 1;
  unpack_int4_to_int8_swizzled<CTA_K, head_dim, NUM_THREADS>(sStaging, sK_read, threadIdx.x);

  // Prefetch packed K_1 to staging (will be consumed in first main loop iteration)
  if (num_iterations >= 2 && threadIdx.x == 0)
  {
    expect_bytes<(CTA_K * head_dim / 2) * sizeof(int8_t)>(&barrier_K);
    load_async_4D(sStaging, &tensorMapK_packed, &barrier_K, 0, 1 * CTA_K, kv_head_id, batch_id);
  }

  // ==================== Main loop (overlapped unpack with WGMMA QK) ====================
  for (uint32_t iter = 1; iter < num_iterations; iter++)
  {
    float dequant_scale = q_scale * K_scale[k_scale_idx + (iter - 1) * k_scale_advance_offset];
    sm_scale = original_sm_scale * dequant_scale;

    // QK: wgmma_arrive fences prior sK_read writes from previous iteration's unpack
    wgmma::warpgroup_arrive();
#pragma unroll
    for (uint32_t fq = 0; fq < num_tiles_q; fq++)
    {
      int8_t *sQ_local = sQ + fq * 64 * head_dim;
      wgmma::wgmma_s8s8s32<CTA_K, 0, head_dim>(RS[fq], sQ_local, sK_read);
#pragma unroll
      for (int k_it = 1; k_it < num_tiles_qk_inner; k_it++)
      {
        wgmma::wgmma_s8s8s32<CTA_K, 1, head_dim>(RS[fq], &sQ_local[k_it*32], &sK_read[k_it*32]);
      }
    }
    wgmma::warpgroup_commit_batch();

    // === OVERLAP WINDOW: unpack K_{iter} while WGMMA QK executes async ===
    // Wait for packed K_{iter} TMA to arrive in staging
    wait(&barrier_K, k_phase);
    k_phase ^= 1;
    // Unpack to sK_write (different buffer from sK_read that WGMMA is reading)
    unpack_int4_to_int8_swizzled<CTA_K, head_dim, NUM_THREADS>(sStaging, sK_write, threadIdx.x);
    // Prefetch packed K_{iter+1} to staging (staging is free after unpack read it)
    if (iter + 1 < num_iterations && threadIdx.x == 0)
    {
      expect_bytes<(CTA_K * head_dim / 2) * sizeof(int8_t)>(&barrier_K);
      load_async_4D(sStaging, &tensorMapK_packed, &barrier_K, 0, (iter + 1) * CTA_K, kv_head_id, batch_id);
    }
    // === END OVERLAP WINDOW ===
...
#pragma unrol
    ...
    // wait for V
    wait(&barrier_V, v_phase);
    v_phase ^= 1;

    // PV: wgmma_arrive also fences the sK_write writes from overlap window
    float RO_temp[num_tiles_q][num_tiles_v][8];
    wgmma::warpgroup_arrive();
#pragma unroll
    for (uint32_t fq = 0; fq < num_tiles_q; fq++)
...
#pragma unroll
    ...
    // Swap double-buffered sK pointers for next iteration
    int8_t *sK_tmp = sK_read;
    sK_read = sK_write;
    sK_write = sK_tmp;
  }

  // ==================== Final block (peeled, with masking) ====================
  {
    float dequant_scale = q_scale * K_scale[k_scale_idx + (num_iterations - 1) * k_scale_advance_offset];
    sm_scale = original_sm_scale;
...
// ===================== Packed INT4 Host Functions =====================

torch::Tensor qk_int4_packed_sv_f8_accum_f32_attn_inst_buf(
                  torch::Tensor query,
                  torch::Tensor key,
                  torch::Tensor value,
                  torch::Tensor output,
                  ...
                  int return_lse)
{
  ...
  CHECK_DTYPE(query, torch::kUInt8);
  CHECK_DTYPE(key, torch::kUInt8);
  CHECK_DTYPE(value, at::ScalarType::Float8_e4m3fn);
  CHECK_DTYPE(query_scale, torch::kFloat32);
  CHECK_DTYPE(key_scale, torch::kFloat32);
  ...
  const int batch_size = query.size(0);
  const int packed_dim = query.size(3);
  ...
  if (tensor_layout == 0)
  {
    ...
    CHECK_SHAPE(key, batch_size, kv_len, num_kv_heads, packed_dim);
    CHECK_SHAPE(output, batch_size, qo_len, num_qo_heads, head_dim);
    assert(value.size(1) == head_dim);
    assert(value.size(2) == num_kv_heads);
  }
  else
  {
    ...
    CHECK_SHAPE(key, batch_size, num_kv_heads, kv_len, packed_dim);
    CHECK_SHAPE(output, batch_size, num_qo_heads, qo_len, head_dim);
    assert(value.size(2) == head_dim);
    assert(value.size(1) == num_kv_heads);
  }
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_type, DTypeOut, {
            ...
            constexpr int CTA_K = 128;
            constexpr int NUM_THREADS = 128;
            constexpr int PACKED_DIM = HEAD_DIM / 2;
            ...
            CUtensorMap tma_map_Q = create_tensor_map_4D<CTA_Q, PACKED_DIM, false>(reinterpret_cast<int8_t*>(query.data_ptr()), batch_size, num_qo_heads, qo_len, PACKED_DIM, stride_bz_q, stride_h_q, stride_seq_q);
            CUtensorMap tma_map_K = create_tensor_map_4D<CTA_K, PACKED_DIM, false>(reinterpret_cast<int8_t*>(key.data_ptr()), batch_size, num_kv_heads, kv_len, PACKED_DIM, stride_bz_k, stride_h_k, stride_seq_k);
            CUtensorMap tma_map_V = create_tensor_map_4D<HEAD_DIM, CTA_K>(reinterpret_cast<int8_t*>(value.data_ptr()), batch_size, num_kv_heads, HEAD_DIM, value.size(3), stride_bz_v, stride_h_v, stride_d_v);

            auto* kernel = qk_int4_packed_sv_f8_attn_kernel<CTA_Q, CTA_K, NUM_THREADS, HEAD_DIM, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), DTypeOut, mask_mode, RETURN_LSE, false>;
            // sQ + sK[0] + sK[1] + sV + sStaging (double-buffered sK)
            size_t sMemSize = CTA_Q * HEAD_DIM * sizeof(int8_t) + 2 * CTA_K * HEAD_DIM * sizeof(int8_t) + CTA_K * HEAD_DIM * sizeof(int8_t) + CTA_K * PACKED_DIM * sizeof(int8_t);
            cudaFuncSetAttribute(
                kernel,
                cudaFuncAttributeMaxDynamicSharedMemorySize, sMemSize);
...
torch::Tensor qk_int4_packed_sv_f8_accum_f32_fuse_v_scale_attn_inst_buf(
                    torch::Tensor query,
                    torch::Tensor key,
                    torch::Tensor value,
                    torch::Tensor output,
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            auto* kernel = qk_int4_packed_sv_f8_attn_kernel<CTA_Q, CTA_K, NUM_THREADS, HEAD_DIM, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), DTypeOut, mask_mode, RETURN_LSE, true>;
            // sQ + sK[0] + sK[1] + sV + sStaging (double-buffered sK)
            size_t sMemSize = CTA_Q * HEAD_DIM * sizeof(int8_t) + 2 * CTA_K * HEAD_DIM * sizeof(int8_t) + CTA_K * HEAD_DIM * sizeof(int8_t) + CTA_K * PACKED_DIM * sizeof(int8_t);
            cudaFuncSetAttribute(
                kernel,
