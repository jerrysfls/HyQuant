...
template <uint32_t head_dim, uint32_t BLOCK_SIZE, uint32_t num_pack_per_thread = 1, bool has_sm_scale = false, bool sub_mean = false, QuantType quant_type = QuantType::kInt8, typename T>
...
#pragma unroll
  ...
  constexpr float quant_max = (quant_type == QuantType::kInt8) ? 127.0f : 7.0f;
  ...
  if (thread_id == 0)
  {
    ...
    scale_ptr_base[0] = s_amax / quant_max;
  }

  __syncthreads();

  float tmp_scale = quant_max / s_amax;
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        ...
        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, num_pack_per_thread, true, false, QuantType::kInt8, c_type><<<grid, block>>>(
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        ...
        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, num_pack_per_thread, false, false, QuantType::kInt8, c_type><<<grid, block>>>(
          reinterpret_cast<c_type*>(input.data_ptr()),
          nullptr,
          output.data_ptr<int8_t>(),
          reinterpret_cast<float*>(scale.data_ptr()),
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        ...
        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, num_pack_per_thread, false, true, QuantType::kInt8, c_type><<<grid, block>>>(
          reinterpret_cast<c_type*>(input.data_ptr()),
    ...
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_WARP_BLOCK_SIZE(warp_block_size, WARP_BLOCK_SIZE, {
        DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
          ...
          QuantInt8Kernel<HEAD_DIM, WARP_BLOCK_SIZE, num_pack_per_thread, false, false, QuantType::kInt8, c_type><<<grid, block>>>(
            reinterpret_cast<c_type*>(input.data_ptr()),
            nullptr,
            output.data_ptr<int8_t>(),
            reinterpret_cast<float*>(scale.data_ptr()),
...
// ==================== INT4 quantization variants ====================

void quant_per_block_int4_cuda(
                torch::Tensor input,
                torch::Tensor output,
                torch::Tensor scale,
                float sm_scale,
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        ...
        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, num_pack_per_thread, true, false, QuantType::kInt4, c_type><<<grid, block>>>(
          reinterpret_cast<c_type*>(input.data_ptr()),
          nullptr,
          output.data_ptr<int8_t>(),
          reinterpret_cast<float*>(scale.data_ptr()),
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        ...
        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, num_pack_per_thread, false, false, QuantType::kInt4, c_type><<<grid, block>>>(
          reinterpret_cast<c_type*>(input.data_ptr()),
          nullptr,
          output.data_ptr<int8_t>(),
          reinterpret_cast<float*>(scale.data_ptr()),
...
void quant_per_block_int4_fuse_sub_mean_cuda(
                torch::Tensor input,
                torch::Tensor mean,
                torch::Tensor output,
                torch::Tensor scale,
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input_dtype, c_type, {
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
        ...
        QuantInt8Kernel<HEAD_DIM, BLOCK_SIZE, num_pack_per_thread, false, true, QuantType::kInt4, c_type><<<grid, block>>>(
          reinterpret_cast<c_type*>(input.data_ptr()),
          reinterpret_cast<c_type*>(mean.data_ptr()),
          output.data_ptr<int8_t>(),
          reinterpret_cast<float*>(scale.data_ptr()),
    ...
    DISPATCH_BLOCK_SIZE(block_size, BLOCK_SIZE, {
      DISPATCH_WARP_BLOCK_SIZE(warp_block_size, WARP_BLOCK_SIZE, {
        DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
          ...
          QuantInt8Kernel<HEAD_DIM, WARP_BLOCK_SIZE, num_pack_per_thread, false, false, QuantType::kInt4, c_type><<<grid, block>>>(
            reinterpret_cast<c_type*>(input.data_ptr()),
            nullptr,
            output.data_ptr<int8_t>(),
            reinterpret_cast<float*>(scale.data_ptr()),
...
__global__ void PackInt4Kernel(const int8_t *__restrict__ input, uint8_t *__restrict__ output,
                               const uint32_t num_tokens,
                               const uint32_t stride_bz_input, const uint32_t stride_seq_input, const uint32_t stride_h_input,
                               const uint32_t stride_bz_output, const uint32_t stride_seq_output, const uint32_t stride_h_output)
{
  constexpr uint32_t packed_dim = head_dim / 2;
  constexpr uint32_t pack_size = 8; // process 8 packed bytes (16 int8 values) per float4
  constexpr uint32_t num_threads_per_token = packed_dim / pack_size;
  ...
  uint32_t token_in_block = thread_id / num_threads_per_token;
  uint32_t pack_idx = thread_id % num_threads_per_token;
  uint32_t token_id = bx * BLOCK_SIZE + token_in_block;

  if (token_id >= num_tokens) return;

  const int8_t *in_ptr = input + batch_id * stride_bz_input + head_id * stride_h_input
                        + token_id * stride_seq_input + pack_idx * pack_size * 2;
  uint8_t *out_ptr = output + batch_id * stride_bz_output + head_id * stride_h_output
                    + token_id * stride_seq_output + pack_idx * pack_size;

  // Load 16 int8 values and pack into 8 uint8 values
  int8_t vals[16];
  *reinterpret_cast<float4*>(&vals[0]) = *reinterpret_cast<const float4*>(in_ptr);

  uint8_t packed[8];
#pragma unroll
  for (int i = 0; i < 8; i++) {
    packed[i] = (static_cast<uint8_t>(vals[2*i] & 0x0F)) | (static_cast<uint8_t>((vals[2*i+1] & 0x0F) << 4));
  }

  *reinterpret_cast<float2*>(out_ptr) = *reinterpret_cast<float2*>(&packed[0]);
}
                ...
                int tensor_layout)
{
  ...
  CHECK_DTYPE(output, torch::kUInt8);
  ...
  DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
    ...
    constexpr int num_threads_per_token = packed_dim / 8;
    constexpr int threads_per_block = BLOCK_SIZE * num_threads_per_token;

    dim3 grid((num_tokens + BLOCK_SIZE - 1) / BLOCK_SIZE, num_heads, batch_size);
    dim3 block(threads_per_block);

    PackInt4Kernel<HEAD_DIM, BLOCK_SIZE><<<grid, block>>>(
      reinterpret_cast<const int8_t*>(input.data_ptr()),
